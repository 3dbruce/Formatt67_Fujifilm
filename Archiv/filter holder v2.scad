...
module plate(width, depth, height, rad1, rad2) {
    ...
    difference(){
      ...
      translate([-width/2+2,-15, 1]) cube([width-4,30,height/2]);
...
module buttons(width,spce=0.1){
  tot_len = 40; // total length of button holder
  but_len = 10; // length of button 
  // spce = space between button and the holder
  
    translate([-width/2+1.5,-tot_len/2,1]) cube([1-spce,tot_len-spce,3-spce]); // button holder
    difference(){
      translate([-width/2-1,-but_len/2,1]) cube([5-spce,but_len-spce,3-spce]); // actual button
      translate([-0.5,0,0]) bajonett_xf1855(rim_r1, rim_r2, plate_h); // riffelung
    };
    
    translate([width/2-2.5,-tot_len/2,1]) cube([1-spce,tot_len-spce,3-spce]); // button holder
    difference(){
      translate([+width/2-4,-but_len/2,1]) cube([5-spce,but_len-spce,3-spce]); // actual button
      translate([+0.5,0,0]) bajonett_xf1855(rim_r1, rim_r2, plate_h); // riffelung
    };
}
// main
...
// add upper plate
//translate ([0, 0, plate_h+D]) rotate ([0, 180, 0]) 
